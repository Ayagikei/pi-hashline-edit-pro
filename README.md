...
Each attached file carries a status line: `[complete, N lines; NEVER call read — it returns the same anchors]` means the section is fully attached — edit directly from the attachment with replace and insert, no read needed. Files attach whole; only files listed as omitted in the footer need `read`.
A coverage line after the header reports complete versus truncated counts and the number of 48KB file-boundary chunks the payload splits into, so a host-side cut never tears a file in the middle. A `[files complete: [...] truncated: [...] omitted: [...]]` line right after it lists every attached complete file, truncated file, and omitted file in one place — check that line instead of scanning sections.
